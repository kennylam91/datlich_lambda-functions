...
func Test_buildPutItem(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid body",
			args: args{eventBody: `{"name":"doctor", "email": "test@example.com", "phone": "0123456", "password": "pass"}`},
			want: map[string]types.AttributeValue{
				...
				"password": &types.AttributeValueMemberS{Value: "pass"},
			},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, field := range []string{"name", "email", "phone", "password"} {
